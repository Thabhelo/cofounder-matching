    ...
    for match in matches:
        ...
        if target_user:
            match_dict = {
                ...
                "stage_alignment_score": match.stage_alignment_score,
                "commitment_alignment_score": match.commitment_alignment_score,
                "working_style_score": match.working_style_score,
...
async def request_introduction(
    ...
    # Rate limit check - max 20 intro requests per day
    yesterday = datetime.utcnow() - timedelta(days=1)
    recent_intros = db.query(Match).filter(
        Match.user_id == current_user.id,
        Match.intro_requested_at.isnot(None),
        Match.intro_requested_at >= yesterday
    ).count()

    if recent_intros >= 20:
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail="Maximum of 20 introduction requests per day. Please try again tomorrow."
        )
